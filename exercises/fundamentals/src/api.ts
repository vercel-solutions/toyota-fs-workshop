...
function generateData() {
  ...
  const priceBadges = ["Below market", "Fair price", "Above market"]

  // Combined city/highway rating for a body style running on gas.
  const mpgByBodyStyle: Record<string, [number, number]> = {
    Sedan: [28, 39],
    Hatchback: [44, 43],
    SUV: [22, 25],
    Truck: [18, 23],
  ...
  // How many mpg the powertrain adds on top of the gas baseline.
  const mpgBonusByPowertrain: Record<string, number> = {
    "Gas": 0,
    "Hybrid": 13,
    "Plug-in Hybrid": 16,
    "Electric": 0,
  ...
  const threeRowModels = ["Highlander", "Grand Highlander", "Sequoia", "Sienna"]
  ...
  const inventory = [
    ...
    { year: 2022, model: "Camry", trim: "LE", price: 22340, mileage: 48215, bodyStyle: "Sedan", powertrain: "Gas", drivetrain: "Front Wheel Drive", segment: "sedans" },
    { year: 2024, model: "Corolla Hybrid", trim: "SE", price: 26410, mileage: 14905, bodyStyle: "Sedan", powertrain: "Hybrid", drivetrain: "All Wheel Drive", segment: "hybrids-evs" },
    { year: 2022, model: "Highlander", trim: "XLE", price: 36490, mileage: 57830, bodyStyle: "SUV", powertrain: "Gas", drivetrain: "All Wheel Drive", segment: "suvs" },
    { year: 2026, model: "4Runner", trim: "Limited", price: 56300, mileage: 2940, bodyStyle: "SUV", powertrain: "Hybrid", drivetrain: "Four Wheel Drive", segment: "suvs" },
    { year: 2021, model: "Tundra", trim: "Limited", price: 39880, mileage: 71420, bodyStyle: "Truck", powertrain: "Gas", drivetrain: "Four Wheel Drive", segment: "trucks" },
    { year: 2024, model: "Land Cruiser", trim: "Limited", price: 68450, mileage: 13180, bodyStyle: "SUV", powertrain: "Hybrid", drivetrain: "Four Wheel Drive", segment: "suvs" },
    { year: 2023, model: "Prius", trim: "LE", price: 26120, mileage: 28340, bodyStyle: "Hatchback", powertrain: "Hybrid", drivetrain: "Front Wheel Drive", segment: "hybrids-evs" },
    { year: 2026, model: "Prius Prime", trim: "XSE", price: 34990, mileage: 4215, bodyStyle: "Hatchback", powertrain: "Plug-in Hybrid", drivetrain: "Front Wheel Drive", segment: "hybrids-evs" },
    { year: 2024, model: "Crown", trim: "XLE", price: 42180, mileage: 22615, bodyStyle: "Sedan", powertrain: "Hybrid", drivetrain: "All Wheel Drive", segment: "sedans" },
    { year: 2023, model: "Corolla Cross", trim: "LE", price: 24690, mileage: 41530, bodyStyle: "SUV", powertrain: "Gas", drivetrain: "Front Wheel Drive", segment: "suvs" },
  ]
  ...
  for (let i = 0; i < 40; i++) {
    ...
    const [city, highway] = mpgByBodyStyle[listing.bodyStyle]
    const mpgBonus = mpgBonusByPowertrain[listing.powertrain]
    ...
    vehicles.push({
      ...
      mpg: listing.powertrain === "Electric"
        ? "131/107"
        : `${city + mpgBonus}/${highway + mpgBonus}`,
      seats: threeRowModels.includes(listing.model) ? 8 : 5,
      segment: listing.segment,
      image: `https://picsum.photos/seed/${slug}/800/600`,
      priceBadge: priceBadges[i % priceBadges.length],
      dealer: dealer.name,
      dealerLocation: dealer.location,
      features,
      certified: listing.year >= 2023 && listing.mileage < 40000,
      apr,
      termMonths,
...
export async function getVehicles(segment?: string): Promise<Vehicle[]> {
  console.info(
    `[API] Fetching vehicles${segment ? ` for segment: ${segment}` : ""} (300ms delay)`,
  )
...
export async function getVehicleBySlug(slug: string): Promise<Vehicle | null> {
  console.info(`[API] Fetching vehicle: ${slug} (200ms delay)`)
...
export async function getFeaturedVehicles(): Promise<Vehicle[]> {
  console.info("[API] Fetching featured vehicles (250ms delay)")
